...
export const EmployeeForm = ({
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    if(values.birthdate && values.email && values.name && values.phone && values.picture) {
    if (validate()) {
  ...
  return (
    <form className="employee-form" onSubmit={handleSubmit} >
      <fieldset>
        ...
        <div>
          <label>Phone:</label>
          <input type="tel" name="phone" value={values.phone} onChange={handleChange} required pattern="[0-9]{10}" maxLength={10} />
        ...
        <div>
          <label>Picture:</label>
          <input type="text" name="picture" value={values.picture} onChange={handleChange} />
